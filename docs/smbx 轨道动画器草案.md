...
- 合法数值 (整数) (类型为 **00**): **[-1600, 14783]** (原始数据取值区间为 [0, 16383] [***0b***0, ***0b***0000-1111-1111-1111]; 计算最终值为原始值 -8191, 所以最终输出的数据取值区间为 [-8191, 8192], 至于为何要如此规划该数值, 是因为 1600 刚好跨两个屏幕宽度 (一屏宽度大概 800), 实践上坐标左右跨两屏应该够用)
- 合法数值 (小数) (类型为 **01**): (-10, 0)&(0, 10) 小数点后的内容, 其分子取值范围为: [-8191,-1]&[1,8191], 分母为恒定的 8191 (表现在二进制上则是 [***0b***0, ***0b***0111-1111-1111-1111], 此处高第二位为 1, 表达该数为小数, 不参与数值运算)
